---
title: "Hanzo Studio Mixing ControlNet Examples"
...
When mixing multiple ControlNets, each ControlNet influences the image generation process according to its applied area. Hanzo Studio enables multiple ControlNet conditions to be applied sequentially in a layered manner through chain connections in the `Apply ControlNet` node:

![apply controlnet chain link](/images/tutorial/controlnet/apply_controlnet_chain_link.jpg)

## Hanzo Studio ControlNet Regional Division Mixing Example
...
![Hanzo Studio Workflow - Mixing ControlNet](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/mixing_controlnets.png)

<Tip>
This workflow image contains Metadata, and can be directly dragged into Hanzo Studio or loaded using the menu `Workflows` -> `Open (ctrl+o)`. The system will automatically detect and prompt to download the required models.
</Tip>

Input pose image (controls the character pose on the left):

![Hanzo Studio Workflow - Mixing ControlNet Input Image](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/mixing_controlnets_input.png)

Input scribble image (controls the cat and scooter on the right):

![Hanzo Studio Workflow - Mixing ControlNet Input Image](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/mixing_controlnets_input_scribble.png)
...
![Hanzo Studio Workflow - Mixing ControlNet Flow Diagram](/images/tutorial/controlnet/flow_diagram_mixing_controlnet.jpg)
