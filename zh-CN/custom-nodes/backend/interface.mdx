...
自定义节点是 Python 代码和可能的模型权重的组合。自定义节点非常强大，允许 Comfy 社区在 Hanzo Studio 中构建自己的功能。

如果您更喜欢阅读代码，可以查看仓库中的[示例](https://github.com/hanzoai/studio/blob/master/custom_nodes/example_node.py.example)。否则，我们将在下面探索 Comfy 的一个内置节点。

## 接口

让我们通过查看 Hanzo Studio 内置的 [Load Checkpoint](https://github.com/hanzoai/studio/blob/master/nodes.py#L529C7-L529C29) 节点来研究自定义节点的接口。这个节点用于加载检查点文件。
...
可选。允许节点控制何时重新执行。Hanzo Studio 尝试仅执行已更改的节点以提高效率。
