---
title: "Hanzo Studio ControlNet 混合使用示例"
...
当我们混合使用多个 ControlNet 时，每个 ControlNet 会根据其应用的区域对图像生成过程施加影响。Hanzo Studio 通过 `Apply ControlNet` 节点的链式连接方式，允许多个 ControlNet 条件按顺序叠加应用混合控制条件：

![apply controlnet chain link](/images/tutorial/controlnet/apply_controlnet_chain_link.jpg)

## Hanzo Studio ControlNet 区域分治混合示例
...
![Hanzo Studio 工作流 - Mixing ControlNet](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/mixing_controlnets.png)

<Tip>
该工作流图片包含 Metadata 数据，可直接拖入 Hanzo Studio 或使用菜单 `Workflows` -> `Open（ctrl+o）` 加载。系统会自动检测并提示下载所需模型。
</Tip>

用于输入的 pose 图片（控制左侧人物姿态）:

![Hanzo Studio 工作流 - Mixing ControlNet 输入图片](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/mixing_controlnets_input.png)

用于输入的 scribble 图片（控制右侧猫咪和滑板车）:

![Hanzo Studio 工作流 - Mixing ControlNet 输入图片](https://raw.githubusercontent.com/hanzoui/example-workflows/main/controlnet/mixing_controlnets_input_scribble.png)
...
![Hanzo Studio 工作流 - Mixing ControlNet 流程图](/images/tutorial/controlnet/flow_diagram_mixing_controlnet.jpg)
