...
by a socket event listener registered in `api.js`. See [messages](/development/hanzo-studio-server/comms_messages).

Messages from the client to the server are sent by the `api.fetchApi()` method defined in `api.js`,
and are handled by http routes defined by the server. See [routes](/development/hanzo-studio-server/comms_routes).
