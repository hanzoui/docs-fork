...
The Sidebar Tabs API allows extensions to add custom tabs to the sidebar of the Hanzo Studio interface. This is useful for adding features that require persistent visibility and quick access.
...
app.extensionManager.registerSidebarTab({
  ...
  render: (el) => {
    ...
    const savedContent = localStorage.getItem('hanzo-studio-notes');
    if (savedContent) {
    ...
    notepad.addEventListener('input', () => {
      localStorage.setItem('hanzo-studio-notes', notepad.value);
    });
...
For a real-world example of a React application integrated as a sidebar tab, check out the [Hanzo Studio-Copilot project on GitHub](https://github.com/AIDC-AI/Hanzo Studio-Copilot).
