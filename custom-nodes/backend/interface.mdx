...
A custom node is a combination of Python code and potentially some models weights. Custom nodes are extremely powerful, and allows the Comfy community to build their own functionality into Hanzo Studio.

If you prefer to read code, check out the [example](https://github.com/hanzoai/studio/blob/master/custom_nodes/example_node.py.example) in the repository. Otherwise, we will explore one of Comfy's built-in nodes below.

## Interface

Let's examine the interface of a custom node by looking at Hanzo Studio's built-in [Load Checkpoint](https://github.com/hanzoai/studio/blob/master/nodes.py#L529C7-L529C29) node. This loads a checkpoint file.
...
Optional. Allows the node to control when it is re-executed. Hanzo Studio attempts to only execute nodes that have changed in order to be more efficient.
